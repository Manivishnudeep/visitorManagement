    ...
    const handleApprove = async () => {
        if (selectedRequest) {
            try {
                ...
                setCheckInRequests(CheckOutRequests.map(req => req._id === selectedRequest._id ? { ...req, status: 'approved' } : req));
    ...
    const handleReject = async () => {
        if (selectedRequest) {
            try {
                ...
                setCheckInRequests(CheckOutRequests.map(req => req._id === selectedRequest._id ? { ...req, status: 'rejected' } : req));
                setSelectedRequest(null); // Close the modal
            } catch (error) {
